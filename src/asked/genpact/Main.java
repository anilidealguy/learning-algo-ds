...
public class Main {
    ...
    private void printCorrections(String writingSample, String wordList) {
        ...
        Set<String> correctStrings = new HashSet<>(parseWords(wordList));
        Set<String> inputStrings = new HashSet<>(parseWords(writingSample));

        inputStrings.removeAll(correctStrings);

        for (String i : inputStrings) {
            float bestScore = 0.0f;
            String bestPossibleWord = "";

            for (String c : correctStrings) {
                float compareResult = fuzzyCompare(i, c);

                if (compareResult > bestScore) {
                    bestPossibleWord = c;
                    bestScore = compareResult;
                }
            }

            System.out.println("Replace \"" + i + "\" with \"" + bestPossibleWord + "\"");
        }
